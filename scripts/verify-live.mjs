...
import { underLinked, duplicateHeadMetadata, leadsWithItsSubject, llmsHostsAgree, openingFigure, PROSE_ROUTES, advertisedReadingsExist, typographyFaults } from "./checks.mjs";
...
console.log("\n5. sitemap");
{
  ...
  /* THE DISCLOSURE PAGE'S SUMMARY AGAINST ITS OWN TABLE.

     /privacy shipped a description reading "cookieless page-view analytics ... one preference
     cookie" above a table listing THREE cookies and a local-storage key. It was written when
     Cloudflare's cookieless beacon was the only counter on the site; GA4 came back on 27 August,
     the table gained two rows, and the sentence describing the table did not. On a page whose
     entire job is stating what the site puts on a reader's machine, an understated summary is
     the page being wrong about the one thing it exists for — and it is the summary, not the
     table, that search results and assistants quote.

     The count is computed from the table's own rows now, so the two cannot drift. This asserts
     that on the SERVED page rather than trusting the source: a derivation that is correct in the
     template and wrong at the edge is the defect this whole file exists to catch. */
  {
    const res = await fetchAs("/privacy", "Mozilla/5.0");
    const desc = /<meta name="description" content="([^"]*)"/.exec(res.body)?.[1] ?? "";
    /* The cookie table is the one with an Expires column; find it by its header rather than by
       its position, so inserting a table above it does not silently move this check. */
    const table = (res.body.match(/<table[\s\S]*?<\/table>/g) ?? [])
      .find((t) => /<th[^>]*>\s*Expires\s*<\/th>/.test(t)) ?? "";
    const kinds = [...table.matchAll(/<td[^>]*>((?:Cookie|Local storage)[^<]*)<\/td>/g)].map((m) => m[1]);
    const cookies = kinds.filter((k) => k.startsWith("Cookie")).length;
    const storage = kinds.filter((k) => k.startsWith("Local storage")).length;
    const WORD = ["no", "one", "two", "three", "four", "five", "six"];
    const gaps = [];
    if (!kinds.length) gaps.push("the cookie table could not be read at all, so this check is blind");
    if (cookies && !desc.includes(`${WORD[cookies] ?? cookies} cookie`)) {
      gaps.push(`the table lists ${cookies} cookie(s) and the description does not say so`);
    }
    if (storage && !desc.includes(`${WORD[storage] ?? storage} local-storage`)) {
      gaps.push(`the table lists ${storage} local-storage key(s) and the description does not say so`);
    }
    /* The specific words that made the old sentence false. "cookieless" is true of one of the two
       counters and the summary presented it as true of the page. */
    if (/cookieless/i.test(desc) && cookies > 0) {
      gaps.push(`the description calls the analytics "cookieless" while the table lists ${cookies} cookie(s)`);
    }
    gaps.length
      ? bad(`/privacy summary disagrees with its own table: ${gaps.join("; ")}`)
      : ok(`/privacy states what it sets — ${cookies} cookie(s) and ${storage} local-storage key(s) — and the summary a search result quotes carries the same counts as the table`);
  }

  /* THE STATED SITUATION ON EVERY PAGE THAT HAS ONE, checked for the rendering accidents that
     spoil the one sentence a reader is guaranteed to see. Astro puts a whitespace text node
     between adjacent JSX expressions on separate lines, which shipped "on a $10,000 position ."
     to /coins/{coin}. Checked on production because what a reader sees is what the edge served,
     and the templates are shared, so one fault appears on fifty URLs at once.

     ONE PAGE PER TEMPLATE, not per URL: the sentence is generated by the same expression on all
     fifty contract pages, so fifty fetches would find one fault fifty times and cost the site's
     own edge fifty requests to learn nothing more. */
  {
    const ROUTES = ["/funding", "/funding/btc", "/coins", "/coins/bitcoin", "/open-interest",
                    "/liquidations", "/liquidations/eth", "/unlocks"];
    const spoiled = [];
    for (const r of ROUTES) {
      const res = await fetchAs(r, "Mozilla/5.0");
      const faults = typographyFaults(res.body);
      if (faults.length) spoiled.push(`${r} — ${faults[0]}`);
    }
    spoiled.length
      ? bad(`the stated situation reads wrong on ${spoiled.length} template(s): ${spoiled.join("; ")}`)
      : ok(`the stated situation on ${ROUTES.length} templates renders without a gap before punctuation, a welded figure or an unresolved value`);
  }
...
console.log("\n11. page weight on the wire");
{
  const BUDGET = [
    ...
    /* RAISED 11,500 -> 18,000 ON 16 SEPTEMBER, on purpose, in a diff, with the reason — and only
       after the page was made smaller first, which is the order the paragraph above asks for.

       WHAT GREW. The table gained a shape column: seven days of Hyperliquid funding per contract,
       four-hour means, fifty rows on one shared axis. Measured on production the day it shipped,
       the column costs 4,182b gzipped — the page went 11,0xx -> 16,819b.

       WHAT WAS GIVEN BACK BEFORE THIS NUMBER WAS TOUCHED, 2,282b of it. The coordinates were
       written to one decimal over a 0-100 box; the viewBox is now the point count so x is the
       integer index, y is rounded to whole units (0.18px at this row height, below the rendering
       grid), a side with nothing on it is no longer drawn as a flat polygon along the axis, the
       zero rule moved from a repeated <line> into one CSS background, and the per-row aria-label
       dropped the sentence about the shared scale that is stated once under the table anyway.
       16,819 -> 14,537b. The rest is genuinely unique per row: 50 contracts x 42 points is the
       data, and the only way to halve it is to halve the resolution.

       WHY IT IS WORTH THE BYTES. The tables stated a level and nothing about how it got there,
       so a reader could not tell a rate that had held all week from one that arrived this
       morning. That was the whole complaint. 18,000 is the observed 14,537 plus the quarter of
       headroom this list is specified to carry, not a round number picked to stop the alarm. */
    ["/funding", 18_000], ["/funding/btc", 92_000], ["/coins/bitcoin", 72_000],
...
console.log("\n15. the ingest write budget still holds at today's coverage");
{
  ...
  /* THE SENTENCE HAS ONE AUTHOR NOW — coverageToday() in src/lib/coverage.ts, which /about and
     /data-sources both render since 16 September, when the old wording ("clear that floor today;
     50 are published") was confirmed misleading by the claim-vs-table audit. This pattern follows
     that function's wording; if the function changes, this fails loudly, as it did the day it did —
     twice: "are published" became "have pages" when the retained band was added the same evening. */
  const m = /([\d,]+) of the ([\d,]+) perpetuals the venue lists clear the \$[\d.,]+[KMB]? floor today and ([\d,]+) have pages/.exec(
  ...
  if (!m) {
    ...
    /* 15 SINCE 16 SEPTEMBER. The new one writes `spark:funding`, the per-contract funding shape
       the hub and the coins table draw as a column, and it is gated on the four-hour bucket
       boundary having passed: 288 ingest ticks a day, six of which can write it, so at most SIX
       writes a day — 180 a month against a million. It is one key holding all fifty contracts
       rather than one key each, which is the reason it does not enter the per-tick arithmetic
       below alongside the sweeps; a per-symbol version would have added 50 x 6 = 300 a day.
       Raised deliberately, which is the whole point of this assertion: a write added anywhere
       fails this until somebody comes back and decides what it costs. */
    const PUT_SITES_KNOWN = 15;
